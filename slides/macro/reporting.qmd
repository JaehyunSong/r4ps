...
1. `predictions()`（予測値）、`marginaleffects()`（限界効果）関数で予測値、または限界効果を計算し、オブジェクトとして格納
   * 表形式オブジェクトとして格納される。
2. 予測値・限界効果オブジェクトの加工（ラベル付け/factor化）
   * 予測値は`predicted`、限界効果は`dydx`列
...
* 予測値は`predicted`列、95%信頼区間の下限と上限はそれぞれ`conf.low`と`conf.hihg`列
...
* 作図に不要な列が多いため、必要な列のみ残す。
   * `marginaleffects()`で限界効果を出した場合は`predicted`の代わりに`dydx`列
...
* 出場回数（`Times`） = 1、2、3、...全てに対して線を引くと表が読みにくくなるため、出場回数が1回、3回、5回のみの予測値を計算
...
* 列抽出の際、限界効果は`predicted`でなく、`dydx`であることに注意
