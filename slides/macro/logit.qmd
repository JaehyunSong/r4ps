...
* `newdata`で指定されなかった説明変数は中央値に固定される。
   * 論文、レポートでは「XXは1、ZZは5、...に固定し、その他の説明変数は中央値に固定した予測値を算出した」と書く。
   * 詳細は第10回の講義資料を参照
...
* `Knowledge`が`Voted`に与える影響は一定ではない。
   * `Knowledge`の値が1の場合: 投票に参加する確率は`r sprintf("%.2f", fit1_pred$estimate[1] * 100)`%
   * `Knowledge`の値が2の場合: 投票に参加する確率は`r sprintf("%.2f", fit1_pred$estimate[2] * 100)`% (`r sprintf("%.2f", (fit1_pred$estimate[2] - fit1_pred$estimate[1]) * 100)`%p増加)
   * `Knowledge`の値が3の場合: 投票に参加する確率は`r sprintf("%.2f", fit1_pred$estimate[3] * 100)`% (`r sprintf("%.2f", (fit1_pred$estimate[3] - fit1_pred$estimate[2]) * 100)`%p増加)
   * `Knowledge`の値が4の場合: 投票に参加する確率は`r sprintf("%.2f", fit1_pred$estimate[4] * 100)`% (`r sprintf("%.2f", (fit1_pred$estimate[4] - fit1_pred$estimate[3]) * 100)`%p増加)
   * `Knowledge`の値が5の場合: 投票に参加する確率は`r sprintf("%.2f", fit1_pred$estimate[5] * 100)`% (`r sprintf("%.2f", (fit1_pred$estimate[5] - fit1_pred$estimate[4]) * 100)`%p増加)
...
fit1_pred |>
  ...
  geom_segment(aes(x = 4.05, xend = 4.95, y = 0.744, yend = 0.840),
               arrow = arrow(type = "closed"), linewidth = 1, color = "red") +
  annotate("label", x= 1.5, y = 0.4,  
           label = str_c(sprintf("%.3f", fit1_pred$estimate[2] - fit1_pred$estimate[1]), "増加")) +
  annotate("label", x= 2.5, y = 0.55,  
           label = str_c(sprintf("%.3f", fit1_pred$estimate[3] - fit1_pred$estimate[2]), "増加")) +
  annotate("label", x= 3.5, y = 0.67,  
           label = str_c(sprintf("%.3f", fit1_pred$estimate[4] - fit1_pred$estimate[3]), "増加")) +
  annotate("label", x= 4.5, y = 0.79,  
           label = str_c(sprintf("%.3f", fit1_pred$estimate[5] - fit1_pred$estimate[4]), "増加")) +
  labs(x = "主観的な政治知識", 
       y = "投票に参加する確率の予測値") +
  theme_bw(base_size = 14)
```
...
* 例) 政治知識の係数は約0.593
   * 4分の1は約0.148
   * 政治知識が1から2へ変化した場合: 予測値は`r sprintf("%.2f", fit1_pred$estimate[2] - fit1_pred$estimate[1])`上がる。
   * 政治知識が2から3へ変化した場合: 予測値は`r sprintf("%.2f", fit1_pred$estimate[3] - fit1_pred$estimate[2])`上がる。
   * 政治知識が3から4へ変化した場合: 予測値は`r sprintf("%.2f", fit1_pred$estimate[4] - fit1_pred$estimate[3])`上がる。
   * 政治知識が4から5へ変化した場合: 予測値は`r sprintf("%.2f", fit1_pred$estimate[5] - fit1_pred$estimate[4])`上がる。
