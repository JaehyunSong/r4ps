...
* 母集団が3人（A, B, C）で、 $n$ = 2の標本を無作為抽出する方法
   * {A, B}、{B, C}、{A, C}の3通り[^permutation]
   * $_3 C_2 = \frac{3!}{(3-2)!2!} = 3$
* 母集団が10人で、 $n$ = 4の標本を無作為抽出する方法
   * $_{10} C_4 = \frac{10!}{(10-4)!4!} = 210$
* 1億2千万人から $n$ = 3000の標本を無作為抽出する方法
   * 有限ではあるが、実質、無限と考えられる
...
[^permutation]: 厳密に言うと、標本分布は一度選ばれた要素がもう一度選ばれる可能性のある「復元抽出」を使う。つまり、{A, A}や{B, B}といった組み合わせも存在する。また、{A, C}と{C, A}も異なる組み合わせと扱われる。スライドの例は「非復元抽出 & 順番なし」であるが、厳密には「復元抽出 & 順番あり」を使用する。したがって、あり得る組み合わせの数は$3^2$ = 9種類である（{A, A}、{B, B}、{C, C}、{A, B}、{B ,A}、{A, C}、{C, A}、{B, C}、{C, B}）。しかし、通常、母集団は非常に大きいため、両者の違いは無視できる（いずれも実質無限大）。
...
* 標本不偏分散（$u^2$）も母分散の不偏推定量（$\mathbb{E}(u^2) = \sigma^2$）
   * 標本分散（$s^2$）の計算式は母分散（$\sigma^2$）と同じ$\frac{1}{n}\sum (x - \bar{x})^2$であるが、これは母分散と不偏推定量ではない。標本不偏分散（$u^2$）は$\frac{1}{n}$の代わりに$\frac{1}{n - 1}$を使用する。
