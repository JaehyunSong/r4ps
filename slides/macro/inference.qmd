...
* 高槻市長の支持率は?
   * 18歳以上の高槻市民（約29万人）に濱田市長の支持有無を尋ね、支持する人の数を有権者数で割る。
...
* 例) 標本を鳥取市から抽出した場合、そこから得られた自民党の支持率は母集団を代表せず、**偏り**（bias）が生じる。
* 偏りをなくす抽出方法: **単純無作為抽出法**
...
* 母集団からの抽出 $\rightarrow$ 標本平均の計算を繰り返す場合
   * 偏りの**ある**標本の場合、母平均と大きく離れた値が得られる
      * 鳥取市から抽出を行い、標本平均の平均の計算を繰り返しても母平均（日本人全体の特徴）から大きく離れた値が得られる。
      - 標本平均の平均値 $\neq$ 母平均
...
* 母集団のデータがあれば、推定は不要?
* 研究の対象が都道府県、または市区町村、選挙区の場合、これらのデータを入手することは可能
   * 1,718市区町村 = 母集団?
...
* **例1）**母集団が3人（A, B, C）で、 $n$ = 2の標本を無作為抽出する方法
   ...
   * 注意
      1. **復元抽出**：同じ人がもう一度抽出されることがあり得る
      2. **順序あり**：{A, C}と{C, A}は異なる組み合わせ
...
$n = 9$の標本を無作為抽出し、標本平均を計算することを500回繰り返す。
...
sample001 <- sample(height_df$Height, 9)
sample002 <- sample(height_df$Height, 9)
sample003 <- sample(height_df$Height, 9)
...
sample500 <- sample(height_df$Height, 9)
...
for (i in 1:500) {
  temp_sample <- sample(height_df$Height, 9, replace = TRUE)
...
* 標本平均の平均値は母平均と（ほぼ）同じ $\rightarrow$ 標本平均は母平均の不偏推定量
* 標本平均のばらつきは母集団のばらつきよりも小さい（母集団より狭い範囲に分布する）
   * 標本平均のばらつき（= $\bar{x}$の標準偏差 = $x$の**標準誤差**[Standard Error]）は...

$$
\mbox{SE}(x) = \mbox{SD}(\bar{x}) = \sqrt{\frac{\sigma^2}{n}}
$$

* 母分散（$\sigma^2$）が約6.105、標本サイズ（$n$）が9であるため、標準誤差は$\frac{6.105}{\sqrt{9}} \simeq 2.035$（標本平均の標準偏差は約`r sprintf("%.3f", sd(sample_mean))`）
* 標本サイズが大きくなるほど、標本平均と母平均のズレは小さくなり（**大数の弱法則**）、標本平均の標準偏差は小さくなる。
...
# for文で標本抽出&標本平均の計算を繰り返す
for (i in 1:500) {
  temp_sample <- sample(height_df$Height, 4)
  sample_mean1[i] <- mean(temp_sample)
  temp_sample <- sample(height_df$Height, 25)
  sample_mean2[i] <- mean(temp_sample)
}

bind_rows(list("n = 4"   = enframe(sample_mean1),
               "n = 25" = enframe(sample_mean2)), 
          .id = "n") |> 
  mutate(n = fct_inorder(n)) |> 
  ggplot(aes(x = value)) +
  geom_histogram(binwidth = 1, color = "black", fill = "gray80") +
  geom_vline(xintercept = 169.86, color = "red") +
  labs(x = "身長 (cm)", y = "度数") +
  scale_x_continuous(breaks = c(140, 160, 169.86, 180, 200),
                     labels = c(140, 160, 169.86, 180, 200)) +
  coord_cartesian(xlim = c(140, 200)) +
  facet_wrap(~n) +
  theme_bw()
```
...
1. $-1.96 \leq T \leq 1.96$
   * `qt(0.025, df = 1997)`と`qt(0.975, df = 1997)`で計算
2. $-1.96 \leq \frac{\bar{x} - \mu}{\mbox{SE}(x)} \leq 1.96$
3. $-1.96 \cdot \mbox{SE}(x) \leq \bar{x} - \mu \leq 1.96 \cdot \mbox{SE}(x)$
4. $-1.96 \cdot \mbox{SE}(x) - \bar{x} \leq -\mu \leq 1.96 \cdot \mbox{SE}(x) - \bar{x}$
5. $-1.96 \cdot \mbox{SE}(x) + \bar{x} \leq \mu \leq 1.96 \cdot \mbox{SE}(x) + \bar{x}$
6. $\bar{x} - 1.96 \cdot \mbox{SE}(x) \leq \mu \leq \bar{x} + 1.96 \cdot \mbox{SE}(x)$
...
```{r}
#| code-line-numbers: false
t.test(ishin_df$Temp_Ishin, conf.level = 0.79)
```
:::
