...
1. RStudioを起動しておいてください。
2. 本日授業用のプロジェクトを作成するか、既存のプロジェクトを開いておいてください。
3. 授業サポートページから実習用データをダウンロードしておいてください。
   * 実習用データは`Gacha.csv`です。
4. ダウンロードしてデータをプロジェクト・フォルダーに入れておいてください。
   * NIIオンライン分析システムの場合、プロジェクト・フォルダーにアップロード
5. 実習用コードを入力するスクリプト、またはR Markdownを開いておいてください。
...
> 　あるソシャゲにおいて★5のキャラクター（SSR）をガチャで引ける確率は5%であると知られている。そこでプレイヤー100人が行った4772回分のガチャ記録（$n$ = 4772）を入手し、調べた結果、SSRは227回（= 4.757%）出現した。SSRが引けた場合1、引けなかった場合を0とした場合、不偏分散の平方根（$s$）は0.213であった。約0.243%ポイントの差（= 5 - 4.757）ではあるものの、怪しい。このガチャからSSRが引ける確率は本当に5%だろうか。
...
* **参考:** 変数が0または1の値のみを取る場合、これから紹介する「 $t$ 検定」でなく、「母比率の検定」を使うのが適切であるが、ここではより汎用性の高い $t$ 検定の例で紹介する。母比率の検定 (`prop.test()`)についてはサポートページの講義資料   を参照すること。
...
* $p$値は0.430であり、$\alpha$ (= 0.05) 以上であるため、帰無仮説は棄却されない。
   ...
   * SSRキャラクターの出現確率は5%である**とは言えない**。 $\leftarrow \bigcirc$
* 開発元を訴えることは難しいかも知れない。
...
* $t = \frac{\bar{x} - \mu_0}{\frac{s}{\sqrt{n}}}$を考えると、$|t|$が大きくなる条件とは
   1. $|\bar{x}|$が大きい
   2. $s$が小さい
   3. $n$が大きい
