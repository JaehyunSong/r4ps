...
```{r}
#| include: false
pacman::p_load(tidyverse, gt, gtExtras, summarytools, modelsummary,
               marginaleffects)
...
#| fig-width: 7
#| fig-height: 4
predictions(fit1, newdata = datagrid(Satisfaction = 1:5, 
                                     Female       = 0:1)) |>
  mutate(Female = factor(Female, levels = 0:1, 
                         labels = c("男性 (女性ダミーの値 = 0)", 
                                    "女性 (女性ダミーの値 = 1)"))) |>
  ggplot() +
  geom_line(aes(x = Satisfaction, y = estimate, color = Female), linewidth = 1) +
...
predictions(fit2, newdata = datagrid(Satisfaction = 1:5, 
  ...
  ggplot() +
  geom_line(aes(x = Satisfaction, y = estimate, color = Age), linewidth = 1) +
  labs(x = "政治満足度", y = "共産感情温度の予測値 (度)",
       color = "年齢") +
  theme_bw() +
  theme(legend.position = "bottom")
...
* `variables`には説明変数名を指定する（変数名は`"`で囲むこと）
