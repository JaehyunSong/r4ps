...
`Temperature`と`Beer`の相関係数（$\alpha$ = 0.05）
...
* どのように $\alpha$ と $\beta$ の値を推定するか
   ...
   * ordinary least suqaresの略、**OLS**と呼ばれることも多い
...
beer_df %>%
  ...
  ggplot(aes(x = Temperature, y = Beer)) +
  geom_segment(aes(x = 0, xend = 24.3, y = 702, yend = 702),
               linetype = 2) +
  geom_segment(aes(x = 24.3, xend = 24.3, y = 702, yend = 0),
               linetype = 2) +
  geom_point(aes(color = Obs), size = 5) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 1.5) +
  geom_segment(aes(x = 0, xend = 24.3, y = 565.6986, yend = 565.6986),
               linetype = 2) +
  ...
  annotate("text", x = 24.5, y = 634, 
           label = TeX("$e_i$ = 残差"),
           size = 5, hjust = 0) +
  geom_segment(aes(x = 26, xend = 24.3, y = 525, yend = 565.6986),
               arrow = arrow(length = unit(0.075, "inches"), type = "closed")) +
  geom_segment(aes(x = 24.3, xend = 24.3, y = 702, yend = 565.6986),
               arrow = arrow(length = unit(0.1, "inches"), 
                             ends = "both", type = "closed"),
               color = "red") +
  geom_point(aes(x = 24.3, y = 565.6986), size = 2, shape = 1) +
  coord_cartesian(ylim = c(400, 780), xlim = c(7, 30)) +
  scale_y_continuous(breaks = c(565.6986, 702), 
                     labels = c(TeX("$\\hat{y}_i$"), TeX("$y_i$"))) +
  scale_x_continuous(breaks = 24.3, 
                     labels = TeX("$x_i$")) +
  labs(x = "説明変数", y = "応答変数") +
