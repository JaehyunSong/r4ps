...
* `author`: 文書の作成者
* `date`: 日付
  * `` date: `r "\x60r Sys.Date()\x60"` ``に修正すると、Knitした日の日付が自動的につけられる。
...
[教科書第21章](https://www.jaysong.net/RBook/rmarkdown.html)を参照
...
1. 今回課題用のプロジェクトを作成する。
2. LMSの「第5回課題」から「問題ファイル（`.Rmd`形式）」ダウンロードし、プロジェクトのフォルダーにアップローロする。
