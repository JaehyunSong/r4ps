...
* 実引数として渡す箇所に`.` (tidyverse)、`_` (native pipe)を入力
   * ただし、`_`はR 4.2.0以上から使用可能 (NIIオンライン分析システムはR 4.1.2)
...
**例)** `df`から`Continent`が`"Asia"` (条件1)か`"Oceania"` (条件2)であり、`HDI_2018`が0.9以上 (条件3)の行を抽出し、`Country`と`HDI_2018`、`Continent`列を抽出
