...
`NA`でなく、**生成 or 修正される列のデータ型**に応じて`NA_real_`（numeric型）、または`NA_character_`（character型）を使用（logical型なら`NA`のままでOK）
...
#| eval: false
my_data |>
  mutate(Age      = if_else(Age      == 999 ~ NA_real_, TRUE ~ Age),
         HighEduc = if_else(HighEduc == 9   ~ NA_real_, TRUE ~ HighEduc))
```
...
my_data |>
  mutate(Age = if_else(Age == 999 ~ NA_real_, TRUE ~ Age)) |>
  mutate(HighEduc = if_else(HighEduc == 9 ~ NA_real_, TRUE ~ HighEduc))
```