...
　統計的仮説検定（今回は$t$検定）の結果、$p$値は0.430であり、$\alpha$ (= 0.05) 以上であるため、帰無仮説は棄却されない。したがって、「SSRキャラクターの出現確率は5%でない**とは言えない**。」と解釈する。注意してほしいのは「帰無仮説は正しい」、つまり「SSRキャラクターの出現確率は5%である。」と解釈しては行けないことだ。つまり、今回の例で帰無仮説が棄却されず、受容されたということは「開発元の主張が間違っているとは言えない（= 対立仮説が正しいとは言えない）」状態であることを意味する。今回の分析結果に基づくと、開発元を訴えることは難しいかも知れない。
