...
　図に英語と日本語が混在することは良くないので、`mean_df`を`ggplot()`に渡す前に`stone_type`を日本語にリコーディングし、SSR列には100をかけて%に換算する。リコーディングは3値以上であれば、`case_when()`、または`recode()`を使うが、今回は`"Free"`か`"Paid"`かの2値となるので、`ifelse()`を使用する。
