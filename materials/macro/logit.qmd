...
　`datagrid()`内で変数を特定の値で指定しない場合、その変数は自動的に**中央値**が代入される。たとえば、`Knowledge`が1〜5の場合の投票参加確率の予測値を計算してみよう。その他の変数はすべて中央値に固定する。計算結果は`fit1_pred`に格納する。
...
　話を戻そう。これまでの結果を見れば、`Knowledge`が`Voted`に与える影響は一定ではないことがわかる。たとえば、`Knowledge`の値が1の場合、投票に参加する確率は`r sprintf("%.2f", fit1_pred$estimate[1] * 100)`%である。ここで`Knowledge`の値が2の場合、投票に参加する確率は`r sprintf("%.2f", fit1_pred$estimate[2] * 100)`%であり、`r sprintf("%.2f", (fit1_pred$estimate[2] - fit1_pred$estimate[1]) * 100)`%**p**増加したことが分かる（`r sprintf("%.2f", (fit1_pred$estimate[2] - fit1_pred$estimate[1]) * 100)`「%」でなく、`r sprintf("%.2f", (fit1_pred$estimate[2] - fit1_pred$estimate[1]) * 100)`「%p」であることに注意しよう。%pは「パーセントポイント」と読む）。それでは`Knowledge`の値が3の場合はどうだろうか。この場合の投票参加の確率は`r sprintf("%.2f", fit1_pred$estimate[3] * 100)`%であり、先より`r sprintf("%.2f", (fit1_pred$estimate[3] - fit1_pred$estimate[2]) * 100)`%**p**増加した。また、`Knowledge`の値が4の場合の予測確率は`r sprintf("%.2f", fit1_pred$estimate[4] * 100)`%（`r sprintf("%.2f", (fit1_pred$estimate[4] - fit1_pred$estimate[3]) * 100)`%**p**増加）、5の場合のそれは`r sprintf("%.2f", fit1_pred$estimate[5] * 100)`%で、`r sprintf("%.2f", (fit1_pred$estimate[5] - fit1_pred$estimate[4]) * 100)`%**p**増加したことが分かる。このようにロジスティック関数（から得られた曲線）の形が非線形であることを考えると、ある意味、当たり前のことであろう。
...
　そもそも異なるモデルを使用しているため、係数が大きく異なるのは仕方ない。重要なのは予測値/予測確率だろう。今回は主観的な政治知識（`Knowledge`）を1から5まで1単位で動かし、その他の共変量は中央値に固定した上で、投票参加の予測確率を計算してみよう。それぞれ計算結果は`lm_pred`と`lgoit_pred`という名のオブジェクトとして作業環境内に格納する。
